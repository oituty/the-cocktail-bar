<template>
  ...
  <div
    v-else-if="cocktail"
    class="card bg-black border-2 relative flex justify-center border-secondary aspect-[9/12] w-full"
  >
    <div
      class="hover-filter rounded-t-full absolute h-full w-[90%] mx-auto bg-error bg-cover bg-no-repeat bg-center z-0"
      :style="backgroundStyles"
    ></div>
    <div class="z-10 flex flex-col justify-between w-full">
      <NuxtLink :to="`/cocktails/${cocktail.idDrink}`" class="flex-1">
        <h3 class="p-4 !text-secondary font-secondary font-extrabold !text-xl">
          {{ cocktail.strDrink }}
        </h3>
      </NuxtLink>
